import { loadLeagueData, loadWeekMatchups } from "./components/data.js";
...
const routes = ["home","matchups","standings","teams","recaps"];
...
async function start() {
  try {
    ...
    const matchups = await loadWeekMatchups(leagueData.currentWeek, leagueData);
    document.getElementById("ticker-status").textContent =
      `${leagueData.status.toUpperCase()} • WEEK ${leagueData.currentWeek}`;

    await Promise.all([
      renderHome(leagueData),
      renderStandings(leagueData),
      renderTeams(leagueData),
      renderMatchups(matchups, leagueData),
      renderRecaps(),
    ]);
  } catch (error) {
    console.error(error);
    document.getElementById("ticker-status").textContent = "DATA TEMPORARILY UNAVAILABLE";
  }
}
